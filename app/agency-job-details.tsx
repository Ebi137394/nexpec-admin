        ...
        {isPending && (
          <View style={st.modalActions}>
            ...
            <TouchableOpacity style={st.acceptBtn} disabled={updating} onPress={() => onUpdateStatus(applicant.id, applicant.applicant_id, 'accepted', inspectorMessage)}>
              {updating ? <ActivityIndicator size="small" color={C.text} /> : <><ThumbsUp size={16} color={C.text} /><Text style={st.acceptBtnTxt}>Hire Inspector</Text></>}
            </TouchableOpacity>
          </View>
        )}
      </SafeAreaView>
      ...
      if (appErr) {
        ...
        const mapped = appData.map((app: any) => {
          ...
          return {
            ...
            inspector: inspectorProfile || { full_name: 'Unknown Inspector' }
          };
        });
  ...
  const handleUpdateStatus = async (appId: string, inspectorId: string, newStatus: string, message: string) => {
    ...
    try {
      ...
      if (newStatus === 'accepted' && job) {
        // ★ V3 CUTOVER (2026-06-15) — the agency-accept flow no longer
        //   writes a row to the legacy V1 `public.contracts` table.
        //
        //   Under the V3 model, binding contracts are server-side
        //   generated by the NEXPEC admin team via the
        //   `admin_generate_job_contract` RPC, which writes to
        //   `public.job_contracts` with blind-pricing enforcement and
        //   the full state machine (pending_client_signature →
        //   pending_inspector_signature → executed).
        //
        //   Acceptance here transitions the application to 'accepted'
        //   and the job to the next lifecycle state, but contract
        //   issuance is decoupled from this client-side action.
        //   See: supabase/migrations/20260615120000_legacy_contracts_decommission.sql
        //
        //   The legacy insert was removed to prevent duplicate contracts
        //   appearing on the inspector's Hub (one legacy + one V3).
        //   The insert would also fail post-migration because the
        //   table-level REVOKE blocks INSERTs from authenticated.
        //
        //   Kept the cents calculation here as a no-op placeholder so
        //   the surrounding code's logical flow is unchanged.
        void ((job as any).client_price_cents || (job as any).budget_cents || 0);
        ...
        // Saves the message into the jobs table (as a private note) so it isn't lost
      ...
      await fetchData();
      setShowModal(false);
      Alert.alert('Success', newStatus === 'accepted' ? 'Inspector Hired Successfully!' : 'Applicant Rejected.');
    } catch (err: any) {
