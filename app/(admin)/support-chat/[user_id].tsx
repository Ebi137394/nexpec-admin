// app/(admin)/support-chat/[user_id].tsx
// ─────────────────────────────────────────────────────────────
//  Admin Support Chat Room
//  Mirror of app/support-chat.tsx, but the admin sends messages
//  INTO a customer's thread instead of their own. Same dark
...
import {
  ...
  StatusBar, Linking, Keyboard, Dimensions,
} from 'react-native';
import { Stack, useRouter, useLocalSearchParams } from 'expo-router';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useAuth } from '../../../src/contexts/AuthContext';
// ★ Consolidation: canonical supabase client @/lib/supabase — same
...
import {
  ...
  FileText, ChevronDown, ShieldCheck,
...
// Strip any "job_" or stray prefix from a route param so Supabase only
// ever sees a pure UUID. Mirrors the cleanUuid pattern used elsewhere.
...
const displayName = (p: CustomerProfile | null): string => {
...
export default function AdminSupportChatScreen() {
  ...
  // Pull the customer's id off the URL (sanitized).
  const params = useLocalSearchParams<{ user_id: string }>();
  const rawParam = Array.isArray(params.user_id) ? params.user_id[0] : params.user_id;
  const customerId = useMemo(() => cleanUuid(rawParam), [rawParam]);
  ...
  const displayMessages: DisplayMessage[] = useMemo(
    () => [...optimisticMsgs, ...messages],
    [optimisticMsgs, messages]
  );

  // Lazy-resolve image attachments into signed URLs (1-hour TTL).
  const ensureSignedUrls = useCallback(async (msgs: SupportMessage[]) => {
    const need = msgs.filter(
      (m) =>
        m.attachment_type === 'image' &&
        m.attachment_url &&
        !signedUrlCache[m.attachment_url]
    );
    ...
    for (const r of results)
      if (r.status === 'fulfilled' && r.value.url) newEntries[r.value.path] = r.value.url;
    if (Object.keys(newEntries).length > 0)
      setSignedUrlCache((prev) => ({ ...prev, ...newEntries }));
  }, [signedUrlCache]);
  ...
  const handleSendText = useCallback(async () => {
    ...
    if (!body || isSending || !adminId || !customerId) return;
    hapticLight();
    setText('');
    setInputHeight(40);

    const tempId = `optimistic_${Date.now()}_${Math.random()}`;
    const ghost: DisplayMessage = {
      id: tempId,
      user_id: customerId,
      sender_id: adminId,
      content: body,
    ...
    setOptimisticMsgs((prev) => [ghost, ...prev]);
    setIsSending(true);
    try {
      const { error } = await supabase
        .from('helpdesk_messages')
        .insert({ user_id: customerId, sender_id: adminId, content: body });
  ...
  }, [text, isSending, adminId, customerId]);

  // ── Upload + send an attachment ───────────────────────────
  const uploadAndSendAttachment = useCallback(
    async (uri: string, kind: 'image' | 'document', fileName: string) => {
      if (!adminId || !customerId) return;
      hapticLight();
      ...
      try {
        ...
        setOptimisticMsgs((prev) => prev.filter((m) => m.id !== tempId));
        Alert.alert('Upload Failed', err.message ?? 'Could not upload the attachment.');
      } finally {
        setIsSending(false);
      }
    },
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: DisplayMessage; index: number }) => {
      // From the admin's perspective, "isMe" = the admin.
      const isMe = item.sender_id === adminId;
      const below = index > 0 ? displayMessages[index - 1] : null;
      const above =
        index < displayMessages.length - 1 ? displayMessages[index + 1] : null;
      const sameSenderBelow =
        below &&
        below.sender_id === item.sender_id &&
        dayKeyFromIso(below.created_at) === dayKeyFromIso(item.created_at) &&
        Math.abs(
      ...
      const sameSenderAbove =
        above &&
        above.sender_id === item.sender_id &&
        dayKeyFromIso(above.created_at) === dayKeyFromIso(item.created_at) &&
        Math.abs(
          new Date(above.created_at).getTime() -
            new Date(item.created_at).getTime()
        ) < GROUP_THRESHOLD_MS;
      const isTopOfGroup = !sameSenderAbove;
      const isBottomOfGroup = !sameSenderBelow;
      const showDateHeader =
        !above || dayKeyFromIso(above.created_at) !== dayKeyFromIso(item.created_at);
      const showOtherAvatar = !isMe && isBottomOfGroup;

      const myRadii = {
        borderTopLeftRadius: 18,
        borderTopRightRadius: isTopOfGroup ? 18 : 4,
        borderBottomLeftRadius: 18,
        borderBottomRightRadius: isBottomOfGroup ? 18 : 4,
      };
      const theirRadii = {
        borderTopLeftRadius: isTopOfGroup ? 18 : 4,
        borderTopRightRadius: 18,
        borderBottomLeftRadius: isBottomOfGroup ? 18 : 4,
        borderBottomRightRadius: 18,
      };

      let imageDisplayUri: string | null = null;
      if (item.attachment_type === 'image') {
        if (item._optimistic && item._localUri) imageDisplayUri = item._localUri;
        else if (item.attachment_url)
          imageDisplayUri = signedUrlCache[item.attachment_url] ?? null;
      }

      const customerInitials = initialsFor(displayName(customer));
      const customerRoleColor = roleMeta(customer?.role).color;
        ...
        <View>
            ...
            <View style={s.dateSeparator}>
              <View style={s.datePill}>
                <Text style={s.datePillText}>
                  {dateSectionLabel(item.created_at)}
                </Text>
              </View>
            </View>
          )}
          <View
            style={[
              ...
              {
                marginTop: isTopOfGroup ? 10 : 2,
                marginBottom: isBottomOfGroup ? 2 : 0,
          ...
          >
                  ...
                  (customer?.avatar_url ? (
                    ...
                    <View
                      style={[
                        s.msgAvatar,
                        s.msgAvatarFallback,
                        { backgroundColor: customerRoleColor },
                      ]}
                    >
                      <Text style={s.msgAvatarText}>{customerInitials}</Text>
                    </View>
                  ))}
              </View>
            )}
            <View
              style={[
                s.bubble,
                isMe ? [s.bubbleMine, myRadii] : [s.bubbleTheirs, theirRadii],
            ...
            >
              {item.attachment_type === 'image' && (
                <View style={s.attachImageWrap}>
                  {imageDisplayUri ? (
                    ...
                    <View style={s.attachImagePlaceholder}>
              ...
              {item.attachment_type === 'document' && (
                <TouchableOpacity
                  style={s.attachDocRow}
                  activeOpacity={0.7}
                  disabled={item._optimistic}
                  onPress={() =>
                    item.attachment_url && handleOpenDocument(item.attachment_url)
                  }
                >
                  <View
                    style={[
              ...
              <View
                style={[
                  s.msgFooter,
                  isMe ? s.msgFooterMe : s.msgFooterThem,
                ]}
              >
                <Text
                  style={[
                ...
                {item._optimistic && (
                  <ActivityIndicator
                    size={10}
                    color={
                      isMe ? 'rgba(255,255,255,0.5)' : COLORS.textMuted
                    }
                    style={{ marginLeft: 4 }}
                  />
                )}
    ...
    [adminId, displayMessages, signedUrlCache, customer, handleOpenDocument]
  );

  const hasContent = text.trim().length > 0;
  const customerName = displayName(customer);
  const role = roleMeta(customer?.role);

  // ── Bad / missing route param fallback ────────────────────
  if (!customerId) {
    return (
      <View style={[s.root, { paddingTop: insets.top }]}>
        ...
        <View style={s.loadingCenter}>
          <Text style={s.errorText}>Invalid conversation</Text>
        </View>
      </View>
    );
  }
        ...
        <View style={s.headerProfile}>
          ...
          <View style={s.headerInfo}>
            ...
            <View style={s.headerSubRow}>
              ...
              <Text style={s.headerJob} numberOfLines={1}>
                Support thread
              </Text>
            </View>
          </View>
        ...
        {isLoading ? (
          ...
          <View style={s.flex1}>
            <FlatList
              ...
              onScroll={(e) =>
                setShowScrollDown(e.nativeEvent.contentOffset.y > 350)
              }
              ...
              ListEmptyComponent={() => (
                <View style={s.empty}>
                  ...
                  <Text style={s.emptySub}>
                    Send the first reply to {customerName.split(' ')[0]}.
                  </Text>
                </View>
              )}
            />
        ...
        >
          ...
          <View style={s.inputFieldWrap}>
            <TextInput
              ...
              placeholder={`Reply to ${customerName.split(' ')[0]}…`}
          ...
          {hasContent || isSending ? (
            <TouchableOpacity
              ...
              style={[
                s.inputIconBtn,
                (isSending || !hasContent) && { opacity: 0.4 },
              ]}
            >
              {isSending ? (
...
const s = StyleSheet.create({
  ...
  headerJob: { fontSize: 11, color: COLORS.textMuted, flexShrink: 1 },
  rolePill: {
    paddingHorizontal: 6,
    paddingVertical: 2,
    borderRadius: 6,
  ...
  msgFooterThem: { justifyContent: 'flex-start' },
  msgTimeText: { fontSize: 10.5 },
  ...
  attachImage: { width: 210, height: 210, borderRadius: 10 },
  ...
  attachDocText: { marginLeft: 10, flex: 1 },
  attachDocName: { fontSize: 14, fontWeight: '600' },
  attachDocHint: { fontSize: 11, marginTop: 2 },
