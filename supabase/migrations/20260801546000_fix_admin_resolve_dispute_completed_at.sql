-- ════════════════════════════════════════════════════════════════════════════
--  20260801546000_fix_admin_resolve_dispute_completed_at.sql
--
--  DEFECT D25 (P1): the owner's ONLY dispute-resolution path was broken.
--  admin_resolve_dispute's status flip wrote
--      completed_at = CASE WHEN p_resolution = 'completed' ...
--  but public.jobs has NO completed_at column (it does have cancelled_at /
--  cancelled_by / cancel_reason, which admin_cancel_job also writes). Every
--  call therefore died with 42703 regardless of outcome, AFTER passing all the
--  authorization and state gates.
--
--  Why earlier proof missed it: run 4's 32/32 matrix proved the REFUSAL side
--  (role gates and the disputed-state gate raise). The positive resolution was
--  never executed until the run-24 dispute lane did so behaviourally.
--
--  Fix: drop the phantom column write. The canonical completion path
--  (mark_job_completed) writes only status + updated_at; completion time is
--  derivable from updated_at/audit. The cancel branch columns are real and
--  stay. Everything else in the function is byte-identical.
--
--  Rollback: supabase/rollback/20260801546000_rollback.sql (restores the
--  broken body verbatim, for the record).
-- ════════════════════════════════════════════════════════════════════════════
