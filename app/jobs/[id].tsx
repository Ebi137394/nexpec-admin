...
export default function JobDetailScreen() {
  ...
  const handleContractPress = async () => {
    try {
      // 1. Check if a contract exists for this specific job
      const { data: contractData, error } = await supabase
        .from('contracts')
        .select('id')
        .eq('job_id', id) // 'id' comes from useLocalSearchParams
        .maybeSingle();

      if (contractData?.id) {
        // 2. If Found -> Go to the new Contract Screen
        router.push(`/contracts/${contractData.id}`);
      } else {
        // 3. If Not Found -> Show alert (Do not crash)
        Alert.alert(
          'No Contract Yet',
          'The client has not generated a contract for this job yet.',
          [{ text: 'OK' }]
        );
      }
    } catch (err) {
      console.error(err);
      Alert.alert('Error', 'Could not check contract status.');
    }
  };
  ...
  const navigateToInspection = () => {
    router.push(`/jobs/${id}/inspection`);
  };
