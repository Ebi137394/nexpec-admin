...
//  Same fields (severity, category, title, description, site reference, evidence)
//  and the same shared backend RPC (flash_report_create + add_attachment) via the
//  raiseFlashReport server action. No client-side JS — selection state is handled
//  by Tailwind has-[:checked], matching the submit-report form.
//
//  Gating: hired inspector on an active job only. Identity-escrow / price-blind
//  rules are untouched (a flash report carries no payout and no PII).
// ════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <Section
          title="Category"
          subtitle="What kind of issue is this?"
        >
          <fieldset>
        ...
        <div className="flex flex-col items-stretch gap-3 border-t border-white/[0.06] pt-8 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-zinc-500">
            The report is logged immediately and routed to admin. You can track
            and resolve it from the job page.
          </p>
...
/* ─── form primitives (match submit-report) ──────────────────────────────── */
