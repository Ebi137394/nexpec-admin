// ════════════════════════════════════════════════════════════════════════════
//  app/(inspector)/disputes.tsx — Inspector disputes surface
//
//  Mirror of app/(client)/disputes.tsx with two flips:
//    • Eligible-jobs query filters by contractor_id = me (not client_id)
//    • Filing context copy speaks inspector-first
//
//  Same file_dispute RPC, same dispute table, same status state machine.
//  Code is intentionally re-stamped from the client file rather than
//  extracted to a shared component because the buyer + inspector
//  flows are likely to diverge over time (different evidence types,
//  different resolution surfaces). Keeping them as siblings makes
//  future surface-specific tweaks easy without refactoring back from
//  shared abstractions.
// ════════════════════════════════════════════════════════════════════════════
...
export default function InspectorDisputesScreen() {
  ...
  const fetchDisputes = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[inspector-disputes] fetch error:', (err as Error)?.message);
      setItems([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const openFiler = useCallback(async () => {
    ...
    try {
      // Inspector files on jobs they're assigned to (contractor_id = me).
      const { data, error } = await supabase
        .from('jobs')
      ...
      console.warn('[inspector-disputes] jobs fetch error:', (err as Error)?.message);
      setEligibleJobs([]);
    } finally {
  ...
  return (
    <View style={s.root}>
      ...
      <Modal visible={filerOpen} animationType="slide" transparent onRequestClose={() => setFilerOpen(false)}>
        <View style={s.sheetBackdrop}>
          <View style={s.sheet}>
            ...
            <TextInput
              ...
              style={s.bodyInput}
            />
            <Text style={s.bodyCounter}>{body.length} / 8000</Text>

            <Pressable
              onPress={handleFile}
              disabled={filing || !selectedJobId}
              style={({ pressed }) => [
                s.submitBtn,
                (filing || !selectedJobId) && { opacity: 0.5 },
...
const s = StyleSheet.create({
  ...
  catChip: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 9, borderWidth: 1 },
