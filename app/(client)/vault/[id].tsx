// ════════════════════════════════════════════════════════════════════════════
//  app/(client)/vault/[id].tsx — Mobile CLIENT Evidence Vault (detail)
//
//  Web parity for /client/vault/[id]. Owner-scoped by RLS (cdocs_owner_all) — no
//  admin gate; the client only ever resolves their own rows. Opens the file via
//  a 5-min signed URL from the private `client_documents` bucket, or the
//  external link. Uses only columns proven to exist on the live table.
// ════════════════════════════════════════════════════════════════════════════
...
const C = {
  ...
  primary: '#7C3AED', primaryDim: 'rgba(124,58,237,0.14)', red: '#EF4444',
};
...
interface VaultDoc {
  id: string; kind: string; label: string; jobId: string | null; jobTitle: string | null;
  filePath: string | null; externalUrl: string | null; notes: string | null;
  createdAt: string; updatedAt: string;
}

export default function ClientVaultDetailScreen() {
      ...
      {error || !doc ? (
        ...
        <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
          <Animated.View entering={FadeIn.duration(200)} style={{ gap: 16 }}>
            ...
            {doc.jobTitle ? (
              <TouchableOpacity style={s.jobCard} activeOpacity={0.8} onPress={() => doc.jobId && router.push(`/(client)/jobs/${doc.jobId}` as any)}>
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (<View style={s.metaRow}><Text style={s.metaRowLabel}>{label}</Text><Text style={s.metaRowValue}>{value}</Text></View>);
}
