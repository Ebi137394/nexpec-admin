// ════════════════════════════════════════════════════════════════════════════
//  app/inspector/messages/[id]/page.tsx — single conversation view
// ════════════════════════════════════════════════════════════════════════════
...
export default async function InspectorConversationPage({ params, searchParams }: PageProps) {
  ...
  if (!user) redirect('/sign-in?next=' + encodeURIComponent(`/inspector/messages/${id}`));
    ...
    return (
      <div className="flex h-[min(60vh,calc(100vh-7rem))] min-h-[400px] flex-col items-center justify-center rounded-3xl border border-white/[0.06] bg-white/[0.01] p-8 text-center">
        ...
        <p className="mt-2 max-w-md text-sm text-zinc-400">
          The chat room is unavailable right now. Try refreshing your session.
        </p>
  ...
  const returnTo = `/inspector/messages/${id}`;
