// ════════════════════════════════════════════════════════════════════════════
//  app/client/jobs/[id]/index.tsx — LANE-A-PHASE-2.6 route-consolidation stub
//
...
export default function LiteralClientJobsIndexRedirect() {
  const { id } = useLocalSearchParams<{ id: string }>();
  if (!id) return <Redirect href={'/(tabs)/client-dashboard' as any} />;
  return <Redirect href={`/(client)/jobs/${id}` as any} />;
}
