-- ════════════════════════════════════════════════════════════════════════════
--  20260703120000_global_search_rpc.sql
--
--  Sprint 13.4 — Global Search (Cmd+K)
--
--  Implements a permission-aware cross-entity search over:
--    1. inspectors (public.inspectors_directory view — shipped 13.2)
--    2. jobs (public.jobs)
--    3. scope templates (public.inspection_scope_templates)
--
--  Postgres FTS via ts_rank + websearch_to_tsquery. No new tsvector
--  columns — we compute the haystack inline because:
--    (a) the data volume is small (thousands not millions of rows)
--    (b) avoiding new columns means no triggers + no backfill required
--    (c) Postgres caches plans, so subsequent searches stay fast
--  When the corpus grows past ~100k rows per entity, drop in stored
--  tsvector columns + GIN indexes per entity.
--
--  PERMISSION MODEL (enforced inside the RPC):
--    • Inspectors:       reads the inspectors_directory view directly.
--                        That view is already RLS-public-safe.
--    • Jobs:             reads public.jobs but constrains to rows the
--                        caller can see — admin reads all, the assigned
--                        inspector reads their assignments, the client
--                        reads their posts.
--    • Scope templates:  reads inspection_scope_templates WHERE
--                        is_active = true. Public to all authenticated
--                        users (matches the existing /admin/scope-templates
--                        consumer pattern).
--
--  RETURN SHAPE:
--    jsonb[] rows of the form
--      { kind: 'inspector' | 'job' | 'scope_template',
--        id: uuid|text, title, subtitle, href, score }
--    sorted by ts_rank within each kind, the top-K per kind, K=8 default.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  -- Empty query → empty result (avoid expensive table scans).
  ...
  -- Determine caller role for the jobs permission branch.
  ...
  /* ─── 2) Jobs ───────────────────────────────────────────────────── */
  -- Admin reads all; client reads their posts; assigned inspector reads
  -- their assignments. Anonymous viewers get an empty bucket here.
  IF v_user IS NOT NULL THEN
    SELECT coalesce(jsonb_agg(row_to_jsonb(r) ORDER BY r.score DESC), '[]'::jsonb)
      INTO v_jobs
    FROM (
      SELECT
        ...
        CASE
          ...
          WHEN j.assigned_inspector_id = v_user             THEN '/inspector/jobs/' || j.id::text
      ...
      WHERE
        (
          ...
          OR j.assigned_inspector_id = v_user
...
COMMENT ON FUNCTION public.global_search(text, integer) IS
  'Permission-aware cross-entity FTS across inspectors_directory, jobs, '
  'and inspection_scope_templates. SECURITY DEFINER with viewer-role + '
  'auth.uid() gating inside. Returns top-K (default 8) per kind sorted '
  'by ts_rank score.';
