// ════════════════════════════════════════════════════════════════════════════
//  app/(admin)/communications/operational-room/[conversation_id].tsx
//  Admin READ-ONLY transcript of one OPERATIONAL room
//  (Supplier ↔ Inspector, or Buyer ↔ Supplier).
//
//  ── THE INVISIBILITY REQUIREMENT ───────────────────────────────────────────
//  Admin must see everything and leave nothing. This screen therefore:
//    • reads admin_operational_messages_view, never public.messages
...
// One transcript screen serves both operational channels, so the party words
// come from the row rather than being hard-coded.
const partyWord = (p: string) =>
  p === 'inspector' ? 'Inspector' : p === 'supplier' ? 'Supplier' : 'Buyer';
...
interface AdminMessage {
  ...
  sender_party: 'buyer' | 'supplier' | 'inspector';
...
export default function AdminOperationalRoomScreen() {
    ...
    try {
      ...
      const [{ data: head, error: headErr }, { data: rows, error: rowsErr }] = await Promise.all([
        supabase
          ...
          .select('job_id, job_title, job_status, channel, rfq_id, party_a_name, party_a_role, supplier_name')
          .eq('conversation_id', conversationId)
          .maybeSingle(),
        supabase
          .from('admin_operational_messages_view')
          .select('*')
          .eq('conversation_id', conversationId)
      ...
      <View key={m.id} style={styles.entry}>
        <View style={styles.entryHead}>
          ...
          <Text style={[styles.partyName, { color: tint }]} numberOfLines={1}>
            {m.sender_name ?? partyWord(m.sender_party)}
          </Text>
          <Text style={styles.partyRole}>{partyWord(m.sender_party).toUpperCase()}</Text>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.headerSub} numberOfLines={1}>
            {(meta?.party_a_name ?? partyWord(meta?.party_a_role ?? 'buyer'))} ↔ {(meta?.supplier_name ?? 'Supplier')}
      ...
      {meta && (
        <View style={styles.revokedBar}>
          ...
          <Text style={styles.revokedText}>
            {meta.channel === 'buyer_supplier' ? 'Buyer ↔ Supplier' : 'Supplier ↔ Inspector'}
            {meta.rfq_id ? ' · linked RFQ' : ''}
            {'  — these channels do not depend on the buyer\u2019s identity mode.'}
          </Text>
        </View>
      )}
