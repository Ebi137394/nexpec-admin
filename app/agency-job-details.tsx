  ...
  const handleUpdateStatus = async (appId: string, inspectorId: string, newStatus: string, message: string) => {
    ...
    try {
      ...
      if (newStatus === 'accepted' && job) {
        // ★ Task 4: integer cents end-to-end. The contracts table column is
        //   `total_amount_cents` (renamed). The legacy "amount" key here is
        //   informational; we keep it but also write the canonical column.
        const clientPaysCents = (job as any).client_price_cents || (job as any).budget_cents || 0;

        await supabase.from('contracts').insert({
          job_id: id,
          client_id: user?.id,
          inspector_id: inspectorId,
          total_amount_cents: clientPaysCents,
          status: 'draft',
